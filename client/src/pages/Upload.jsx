...
const BLANK = { vendor:'', invoice_no:'', amount:'', gst:'', total:'', description:'', date: new Date().toISOString().slice(0,10), category:'', suggestedCategory:'', source:'', project:'', isReimbursement: false, reimburseTo:'' };
...
export default function Upload() {
  ...
  useEffect(() => {
    Promise.all([getProjects(), getEmployees()]).then(([p, e]) => {
      setProjects(p.data); setEmployees(e.data);
    });
  }, []);
  ...
  const reset = () => {
    setStep(0); setImage(null); setImageFile(null); setScanning(false); setScanPct(0);
    setForm(BLANK); setNewProjMode(false); setNewProjName(''); setSaved(null); setError('');
  };
    ...
    <div className="p-4 md:p-6 max-w-lg mx-auto page-enter">
      ...
      {step === 1 && (
        <div>
          ...
          <div className="relative rounded-2xl overflow-hidden bg-gray-900" style={{ minHeight: 280 }}>
            ...
            <div className="absolute left-0 right-0 h-0.5" style={{ top: `${scanPct}%`, background: '#818cf8', boxShadow: '0 0 16px 4px #818cf8, 0 0 4px 1px white', transition: 'none' }} />
          ...
          <p className="text-center text-xs text-gray-400 mt-3">Using Google Cloud Vision AI for accurate extraction</p>
        </div>
      )}
      ...
      {step === 2 && (
        <div>
          ...
          <div className={`border rounded-xl px-3 py-2 text-xs flex items-center gap-2 mb-4 ${form.source === 'manual' ? 'bg-amber-50 border-amber-100 text-amber-700' : 'bg-indigo-50 border-indigo-100 text-indigo-700'}`}>
            ...
            {form.source === 'manual'
              ? 'OCR not configured — please fill in the details manually. Set GOOGLE_SERVICE_ACCOUNT_KEY to enable AI extraction.'
      ...
      {step === 3 && (
        <div>
          ...
          <div className="space-y-2">
            {projects.map(p => (
              <button key={p.id} onClick={() => { setF('project', p.name); setNewProjMode(false); }}
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border-2 transition text-left ${form.project === p.name && !newProjMode ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200 hover:border-gray-300'}`}>
                <FolderOpen size={17} className={form.project===p.name&&!newProjMode ? 'text-indigo-600':'text-gray-400'} />
                <span className={`text-sm font-medium ${form.project===p.name&&!newProjMode ? 'text-indigo-700':'text-gray-700'}`}>{p.name}</span>
                {form.project===p.name&&!newProjMode && <Check size={15} className="ml-auto text-indigo-600" />}
              </button>
            ))}
      ...
      {step === 4 && (
        <div>
            ...
            {form.isReimbursement && (
              <div>
                ...
                <input
                  ...
                  value={form.reimburseTo}
                  onChange={e => setF('reimburseTo', e.target.value)}
                  className="input"
                />
              </div>
            )}
          ...
          <button onClick={handleSave} disabled={saving || !form.category || (form.isReimbursement && !form.reimburseTo)}
            ...
            {saving ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> Saving & Uploading…</> : <><Check size={16} /> Save Expense</>}
          </button>
        </div>
      )}
